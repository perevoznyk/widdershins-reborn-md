...
describe('common tests', () => {
  describe('slugify tests', () => {
    ...
    it('should replace remaining non-word characters and dashes with a single dash (-)', () => {
      assert.equal(
        common.slugify('id: one, two & three'),
        'id-one-two-and-three'
      );
    });
  ...
  describe('schemaToArray tests', () => {
    ...
    it('should create a row for each property and subproperty', () => {
      ...
      const options = {};
      assert.equal(
        common.schemaToArray(schema1, offset, options, data)[0].rows.length,
        1
      );
      assert.equal(
        common.schemaToArray(schema2, offset, options, data)[0].rows.length,
        6
      );
    });
    ...
    it('should calculate depth properly', () => {
      ...
      const options = {};
      assert.equal(
        common.schemaToArray(schema1, offset, options, data)[0].rows[0].depth,
        1
      );
    ...
    it('should create a name for each row', () => {
      ...
      const options = {};
      assert.equal(
        common.schemaToArray(schema1, offset, options, data)[0].rows[0].name,
        'firstName'
      );
      const result = common.schemaToArray(schema2, offset, options, data);
