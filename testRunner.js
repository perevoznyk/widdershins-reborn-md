...
var argv = require('yargs')
  ...
  .boolean('noschema')
  .alias('n', 'noschema')
  .describe('noschema', 'Set widdershins-reborn --noschema option')
  .boolean('raw')
  .alias('r', 'raw')
  .describe('raw', 'Set widdershins-reborn --raw option')
  .boolean('resolve')
  .describe('resolve', 'Set widdershins-reborn --resolve option')
  .count('verbose')
  .alias('v', 'verbose')
      ...
      if (result != '' && ok) {
        ...
        if (src.info) {
          ...
          console.log(
            '  %s',
            src.host ||
              (src.servers && src.servers.length ? src.servers[0].url : null) ||
              'localhost'
          );
        } else {
...
if (stat && stat.isFile()) {
  ...
  rf(
    pathspec,
    { readContents: false, filenameFormat: rf.FULL_PATH },
    function (err) {
