...
var argv = require('yargs')
  .usage('widdershins-reborn [options] {input-file|url} [[-o] output markdown]')
  .demand(1)
  .strict()
  .string('abstract')
  .alias('a', 'abstract')
  .describe(
    'abstract',
    'The filename of the Markdown include file to use for the ReSpec abstract section.'
  )
  ...
  .describe(
    'clipboard',
    'Set the value of the `code_clipboard` parameter in the header so Markdown processors like Slate include clipboard support in their output.'
  )
  .boolean('code')
  .alias('c', 'code')
  ...
  .describe(
    'headings',
    'Set the value of the `headingLevel` parameter in the header so markdown processors know how many heading levels to show in the table of contents.'
  )
  .default('headings', 2)
  .boolean('httpsnippet')
  ...
  .describe(
    'includes',
    'List of files to put in the `include` header of the output Markdown.'
  )
  .boolean('lang')
  .alias('l', 'lang')
  .describe(
    'lang',
    "Generate the list of languages for code samples based on the languages used in the source file's `x-code-samples` examples."
  )
  .array('language_tabs')
  .describe(
    'language_tabs',
    'List of language tabs for code samples using "language[:label[:client]]" format, such as `javascript:JavaScript:request`.'
  )
  .number('maxLevel')
  .alias('m', 'maxDepth')
  ...
  .describe(
    'respec',
    'Filename containing the ReSpec config object; implies html and omitHeader.'
  )
  .boolean('search')
  .alias('s', 'search')
  .default('search', true)
  .describe(
    'search',
    'Set the value of the `search` parameter in the header so Markdown processors like Slate include search support in their output.'
  )
  ...
  .describe(
    'sotd',
    'The filename of the markdown include file to use for the ReSpec SotD section.'
  )
