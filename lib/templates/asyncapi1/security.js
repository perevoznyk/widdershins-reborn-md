...
module.exports = function anonymous(data) {
  ...
  for (var s in data.api.components.securitySchemes) {
    ...
    if (sd.type == 'http') {
      out += '\n* HTTP ' + sd.scheme;
      if (sd.bearerFormat) {
        out += ' Format: ' + sd.bearerFormat;
      }
      out += ' Authentication. ' + (sd.description || '') + '\n';
    }
    out += '\n';
    if (sd.type == 'userPassword') {
    ...
    out += '\n';
    if (sd.type == 'symmetricEncryption') {
      out += '\n* Symmetric encryption. ' + (sd.description || '') + '\n';
    }
    out += '\n';
    if (sd.type == 'asymmetricEncryption') {
