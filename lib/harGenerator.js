...
/** Generate a HAR request object from operation data. @param {object} data - Operation data. @returns {object} HAR request object. */
function generate(data) {
  const effectiveBaseUrl =
    data.baseUrl == '//' ? 'https://example.com' : data.baseUrl;
  const url = data.requiredUriExample
    ? `${effectiveBaseUrl}${data.requiredUriExample}`
    : data.url;
  ...
  if (data.bodyParameter && data.bodyParameter.present) {
    ...
    if (request.postData.mimeType === 'multipart/form-data') {
      request.postData.params = Object.keys(
