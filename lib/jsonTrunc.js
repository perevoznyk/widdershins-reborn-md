...
let array_handler = {
  ...
  add_visited: node => {
    node.visited++;
  }, // assuming the standard visiting order for array elements, k===node.visited holds (hence the visited index k is useless as parameter here)
...
let object_handler = {
  ...
  add_visited: (node, k) => {
    node.visited.push(k);
  }, // push property k on the array of visited properties of node.value
...
function shallow_copy(v) {
  ...
  for (const prop of Object.getOwnPropertyNames(v)) {
    ...
    Object.defineProperty(
      copy,
      prop,
      desc.get ? { value: getter_trunc, enumerable: true } : desc
    );
  }
  return copy;
}
...
function stringifyTrunc(value, replacer, indent, options) {
  ...
  function replace(k, v) {
    function is_child(c, p, k) {
      // c: child value to be inserted with associated value k; p: potential parent; handler h globally defined in replace
      return p.value[k] === c && h.not_in_visited(k, p.visited);
    }

    function try_push(v) {
      // handler h globally defined in replace; array a and int value depth globally defined in stringifyTrunc
      if (a.length >= depth) return object_trunc;
      if (exit_cycles && a.some(el => el.value === v)) return cycle_trunc;
      a.push({ value: v, visited: h.init_visited() }); // push the node, no keys inspected so far
      return shallow_copy(v);
    }

    function insert(k, v) {
      // handler h and array a globally defined in replace and stringifyTrunc, respectively; return false iff node has not been inserted because of truncation
      while (a.length > 0) {
