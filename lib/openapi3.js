...
function safeUrlParse(urlStr) {
  try {
    ...
    try {
      return new URL(urlStr, 'http://placeholder');
    } catch (e2) {
...
/** Convert an OpenAPI 3.x source definition into a table-of-contents structure. @param {object} source - Parsed OpenAPI 3.x source. @param {object} data - Template data containing options and translations. @returns {object} Resources object keyed by tag name. */
      ...
      for (var m in source.paths[p]) {
        if (
          ...
          if (method.operation.tags && method.operation.tags.length > 0) {
            var tagData = getTagGroup(
              method.operation.tags[0],
              data.options.tagGroups
            );
...
/** Resolve a tag name to its group title and description. @param {string} tag - Tag name to look up. @param {Array<object>} [tagGroups] - Optional array of tag group definitions. @returns {{name: string, description: string}} Resolved group info. */
      ...
      if (!data.bodyParameter.exampleValues.object) {
        ...
        if (op.requestBody.content[rb].schema) {
          ...
          if (schema['x-widdershins-oldRef']) {
            data.bodyParameter.refName = schema['x-widdershins-oldRef'].replace(
              '#/components/schemas/',
              ''
            );
          } else {
            if (
              ...
              data.bodyParameter.refName = schema.items[
                'x-widdershins-oldRef'
        ...
        if (
          op.requestBody.content[rb].examples &&
          Object.keys(op.requestBody.content[rb].examples).length
        ) {
          let key = Object.keys(op.requestBody.content[rb].examples)[0];
          data.bodyParameter.exampleValues.object =
            op.requestBody.content[rb].examples[key].value;
          data.bodyParameter.exampleValues.description =
            op.requestBody.content[rb].examples[key].description;
        } else if (op.requestBody.content[rb].example) {
          data.bodyParameter.exampleValues.object =
            op.requestBody.content[rb].example;
        } else {
          data.bodyParameter.exampleValues.object = common.getSample(
            op.requestBody.content[rb].schema,
            data.options,
        ...
        if (typeof data.bodyParameter.exampleValues.object === 'object') {
          data.bodyParameter.exampleValues.json = common.safejson(
            data.bodyParameter.exampleValues.object,
            null,
            2
          );
...
function getParameters(data) {
  ...
  for (let param of data.parameters) {
    ...
    if (pSchema && !param.safeType) {
      ...
      param.safeType = common.normalizeType(pSchema);
      if (pSchema.format && !Array.isArray(pSchema.type)) {
        param.safeType = param.safeType + '(' + pSchema.format + ')';
      }
      if (param.safeType.startsWith('array') && pSchema.items) {
        let itemsType = common.normalizeType(pSchema.items);
        if (pSchema.items.format) {
          itemsType = itemsType + '(' + pSchema.items.format + ')';
        }
        param.safeType = 'array[' + itemsType + ']';
      }
      if (pSchema['x-widdershins-oldRef']) {
        let schemaName = pSchema['x-widdershins-oldRef'].replace(
          '#/components/schemas/',
          ''
        );
        param.safeType =
          '[' + schemaName + '](#schema' + schemaName.toLowerCase() + ')';
      }
      if (param.refName)
        param.safeType =
          '[' + param.refName + '](#schema' + param.refName.toLowerCase() + ')';
    }
    if (pSchema) {
      ...
      if (typeof param.exampleValues.object === 'object') {
        param.exampleValues.json = common.safejson(
          param.exampleValues.object,
          null,
          2
        );
    ...
    if (param.in === 'path') {
      ...
      uriTemplateStr = uriTemplateStr
        .split('{' + param.name + '}')
        .join(template);
      requiredUriTemplateStr = requiredUriTemplateStr
        .split('{' + param.name + '}')
        .join(template);
    }
      ...
      if (data.api.components.securitySchemes[ess]) {
        ...
        if (
          ...
          (secScheme.type === 'oauth2' ||
            secScheme.type === 'openIdConnect' ||
            (secScheme.type === 'http' &&
              (secScheme.scheme === 'bearer' || secScheme.scheme === 'basic')))
        ) {
          ...
          authHeader.exampleValues.json =
            "'" + authHeader.exampleValues.object + "'";
  ...
  data.requiredQueryString = data.requiredUriExample.substr(
    data.requiredUriExample.indexOf('?')
  );
...
function getBodyParameterExamples(data) {
  ...
  if (data.bodyParameter.schema && data.bodyParameter.schema.xml) {
    ...
    data.bodyParameter.schema &&
    data.bodyParameter.schema['x-widdershins-oldRef']
  ) {
    xmlWrap = data.bodyParameter.schema['x-widdershins-oldRef']
      .split('/')
      .pop();
  }
  ...
  if (common.doContentType(data.consumes, 'xml') && typeof obj === 'object') {
    ...
    content += '```xml\n';
    content +=
      xml.getXml(JSON.parse(common.safejson(obj)), '@', '', true, '  ', false) +
      '\n';
    content += '```\n\n';
  }
  return content;
...
/** Synthesize a body parameter entry from a requestBody schema. @param {object} data - Operation data containing bodyParameter. @returns {object[]} Body parameters array. */
function fakeBodyParameter(data) {
  ...
  if (data.bodyParameter.schema) {
    ...
    if (
      ...
      (data.options.expandBody || !param.schema['x-widdershins-oldRef'])
    ) {
      let offset = data.options.omitBody ? -1 : 0;
      let props = common.schemaToArray(
        data.bodyParameter.schema,
        offset,
        { trim: true },
        data
...
function getResponses(data) {
  ...
  for (let r in data.operation.responses) {
    if (!r.startsWith('x-')) {
      ...
      entry.meaning =
        r === 'default'
          ? data.translations.responseDefault
          : data.translations.responseUnknown;
      var url = '';
      ...
      for (let ct in response.content) {
        ...
        if (
          contentType.schema &&
          contentType.schema['x-widdershins-oldRef'] &&
          contentType.schema['x-widdershins-oldRef'].startsWith('#/components/')
        ) {
          let schemaName = contentType.schema['x-widdershins-oldRef'].replace(
            '#/components/schemas/',
            ''
          );
          entry.schema =
          ...
          if (
            contentType.schema &&
            contentType.schema.type &&
            contentType.schema.type !== 'object' &&
            contentType.schema.type !== 'array'
          ) {
...
function getResponseExamples(data) {
      ...
      for (let ct in response.content) {
        ...
        if (contentType.examples) {
          ...
          if (!autoDone[autoCT]) {
            ...
            examples.push({
              description: resp + ' ' + data.translations.response,
              value: common.getSample(
                obj,
                data.options,
                { skipWriteOnly: true, quiet: true },
                data.api
              ),
              cta: cta,
              xmlWrap: xmlWrap,
  ...
  for (let example of examples) {
    ...
    if (
      typeof xmlObj === 'object' &&
      common.doContentType(example.cta, 'xml')
    ) {
      content += '```xml\n';
      content +=
        xml.getXml(
          JSON.parse(common.safejson(xmlObj)),
          '@',
          '',
          true,
          '  ',
...
function getAuthenticationStr(data) {
  ...
  for (let s in data.security) {
    ...
    for (let sse in Object.keys(data.security[s])) {
      ...
      let sep = count > 0 ? ' & ' : ', ';
      list +=
        (list ? sep : '') + (secDef ? secName : data.translations.secDefNone);
...
function convertInner(api, options) {
  return new Promise(function (resolve, reject) {
    ...
    if (
      data.api.components &&
      data.api.components.schemas &&
      data.api.components.schemas['x-widdershins-oldRef']
    ) {
      delete data.api.components.schemas['x-widdershins-oldRef'];
    }

    if (typeof templates === 'undefined') {
      templates = require('./templates/openapi3');
    }
    ...
    data.version =
      ...
      typeof data.api.info.version === 'string' &&
      data.api.info.version.toLowerCase().startsWith('v')
        ? data.api.info.version
        : 'v' + (data.api.info ? data.api.info.version : 'v1.0.0');

    let header = {};
    header.title = api.info && (api.info.title || 'API') + ' ' + data.version;
    header.language_tabs = options.language_tabs;
    if (options.language_clients)
      header.language_clients = options.language_clients;
    header.toc_footers = [];
    if (api.externalDocs) {
        ...
        header.toc_footers.push(
          '<a href="' +
            ...
            (api.externalDocs.description
              ? api.externalDocs.description
              : data.translations.externalDocs) +
            '</a>'
        );
      }
    }
    ...
    data.title_prefix =
      data.api.info && data.api.info.version
        ? common.slugify((data.api.info.title || '').trim() || 'API')
        : '';
    ...
    data.host = safeUrlParse(data.servers[0].url).host;
    data.protocol = safeUrlParse(data.servers[0].url).protocol;
    ...
    if (options.cleanMarkdown) {
      content = common.stripSchemaPlaceholderSamples(content);
      content = common.cleanMarkdown(content);

      if (!options.omitHeader) {
        const fm = common.buildFrontmatter(api, options);
        const fmString = '---\n' + common.stringifyFrontmatter(fm) + '---\n';
...
/** Convert an OpenAPI 3.x definition to Markdown or HTML. @param {object} api - OpenAPI 3.x definition object. @param {object} options - Conversion options. @returns {Promise<string>} Generated Markdown or HTML. */
